...
class BarChart {
    ...
    calcYMaxVal() {
        let maxVal = Math.max.apply(Math, this.data.map(item => Math.max.apply(Math, item.data)));
        ...
        for(let j = 1; j < this.yAxisIntervalNum + 1; j++) {
            ...
            if(j < this.yAxisIntervalNum) {
                let horLine = this.createLine({
                    range: [[this.origin[0], y], [this.origin[0], y]], 
                    color: '#666'
                });
    ...
    render() {
        ...
        this.yInfo.map(item => {
            this.animateLineLength(item.horLine, this.origin[0] + this.xAxisLength, item.y);
        });
    }
    ...
    animateLineLength(shape, length, y) {
        let that = this;
