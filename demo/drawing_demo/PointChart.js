...
function init3(v, index) {
    ...
    class PointChart {
        ...
        drawTimeline() {
            ...
            this.yearText = new this.r.shapes.Text({
                pin: [this.origin[0] + 450, this.origin[1] - 100],
                content: '1999',
                opacity: 0.4,
                fontSize: 70,
                color: '#ccc'
        ...
        setTimeStep(curTime) {
            let interval = (this.timelineEnd - this.timelineStart)/(this.timelineDotNum),
                text = this.timelineStart + curTime*interval;

            this.yearText.attr('content', text);
    ...
    const generateData = function(length) {
        ...
        return Array.from(new Array(length), (item, i) => {
            ...
            lastPopular = lastPopular + Math.ceil(Math.random()*Math.random()*30000000) + posnav()*500000;
